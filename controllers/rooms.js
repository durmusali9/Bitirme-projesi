...
exports.getAllRooms = async(req, res, next) => {
    try {
        if (mongoose.connection.readyState !== 1) {
            ...
            console.warn('DB not connected, returning mock data for rooms');
            const mockRooms = [{
                    _id: '1',
                    name: 'İngilizce Günlük Konuşma',
                    owner: { name: 'John Doe', email: 'john@example.com' },
                    members: [{ _id: '1', name: 'John Doe' }, { _id: '2', name: 'Jane Smith' }],
                    language: 'english',
                    level: 'intermediate',
                    topic: 'daily',
                    description: 'Günlük hayattan konuşmalar yapıyoruz.',
                    createdAt: new Date()
                },
                {
                    _id: '2',
                    name: 'Başlangıç İngilizce',
                    owner: { name: 'Alice Johnson', email: 'alice@example.com' },
                    members: [{ _id: '3', name: 'Alice Johnson' }],
                    language: 'english',
                    level: 'beginner',
                    topic: 'daily',
                    description: 'Başlangıç seviyesi öğrenenler için ideal.',
                    createdAt: new Date()
                },
                {
                    _id: '3',
                    name: 'İş Dilinde İspanyolca',
                    owner: { name: 'Carlos Garcia', email: 'carlos@example.com' },
                    members: [{ _id: '4', name: 'Carlos Garcia' }, { _id: '5', name: 'Maria Lopez' }, { _id: '6', name: 'Pedro Martinez' }],
                    language: 'spanish',
                    level: 'advanced',
                    topic: 'business',
                    description: 'İş ortamında kullanılan İspanyolca.',
                    createdAt: new Date()
                },
                {
                    _id: '4',
                    name: 'Fransızca Sanat Konuşması',
                    owner: { name: 'Sophie Dubois', email: 'sophie@example.com' },
                    members: [{ _id: '7', name: 'Sophie Dubois' }],
                    language: 'french',
                    level: 'intermediate',
                    topic: 'academic',
                    description: 'Sanat ve kültür hakkında Fransızca tartışmalar.',
                    createdAt: new Date()
                },
                {
                    _id: '5',
                    name: 'Almanca Senaryo Pratiği',
                    owner: { name: 'Hans Mueller', email: 'hans@example.com' },
                    members: [{ _id: '8', name: 'Hans Mueller' }, { _id: '9', name: 'Greta Schmidt' }],
                    language: 'german',
                    level: 'beginner',
                    topic: 'scenario',
                    description: 'Günlük durumlar senaryolarında pratik.',
                    createdAt: new Date()
                }
            ];
            ...
            if (topic) filtered = filtered.filter(r => r.topic && r.topic.includes(topic));

            return res.json({ status: 'success', results: filtered.length, data: { rooms: filtered } });
        }
...
exports.getRoomById = async(req, res, next) => {
    try {
        if (mongoose.connection.readyState !== 1) {
            // Return mock room if DB is not connected
            const mockRoom = {
                _id: req.params.roomId || '1',
                name: 'İngilizce Günlük Konuşma',
            ...
            return res.json({ status: 'success', data: { room: mockRoom } });
        }
